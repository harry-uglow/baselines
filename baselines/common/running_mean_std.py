...
class RunningMeanStd(object):
    ...
    def __init__(self, epsilon=1e-4, shape=()):
        self.mean = np.zeros(shape, u'float64')
        self.var = np.ones(shape, u'float64')
        self.count = epsilon
...
class TfRunningMeanStd(object):
    ...
    '''
    def __init__(self, epsilon=1e-4, shape=(), scope=u''):
        sess = get_session()
        ...
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            self._mean  = tf.get_variable(u'mean',  initializer=np.zeros(shape, u'float64'),      dtype=tf.float64)
            self._var   = tf.get_variable(u'std',   initializer=np.ones(shape, u'float64'),       dtype=tf.float64)
            self._count = tf.get_variable(u'count', initializer=np.full((), epsilon, u'float64'), dtype=tf.float64)
...
def test_tf_runningmeanstd():
    for (x1, x2, x3) in [
        ...
        rms = TfRunningMeanStd(epsilon=0.0, shape=x1.shape[1:], scope=u'running_mean_std' + unicode(np.random.randint(0, 128)))
...
def profile_tf_runningmeanstd():
    ...
    print u'rms update time ({} trials): {} s'.format(n_trials, tic2 - tic1)
    print u'tfrms update time ({} trials): {} s'.format(n_trials, tic3 - tic2)
    ...
    print u'rms get mean time ({} trials): {} s'.format(n_trials, tic2 - tic1)
    print u'tfrms get mean time ({} trials): {} s'.format(n_trials, tic3 - tic2)
