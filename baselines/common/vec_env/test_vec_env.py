...
def assert_venvs_equal(venv1, venv2, num_steps):
    ...
    try:
        ...
        for _ in xrange(num_steps):
            actions = np.array([venv1.action_space.sample() for _ in xrange(venv1.num_envs)])
            for venv in [venv1, venv2]:
                venv.step_async(actions)
            outs1 = venv1.step_wait()
            outs2 = venv2.step_wait()
            for out1, out2 in izip(outs1[:3], outs2[:3]):
...
@pytest.mark.parametrize(u'klass', (ShmemVecEnv, SubprocVecEnv))
@pytest.mark.parametrize(u'dtype', (u'uint8', u'float32'))
def test_vec_env(klass, dtype):  # pylint: disable=R0914
    ...
    fns = [make_fn(i) for i in xrange(num_envs)]
...
class SimpleEnv(gym.Env):
    ...
    def step(self, action):
        ...
        return self._cur_obs, reward, done, {u'foo': u'bar' + unicode(reward)}
...
def test_mpi_with_subprocvecenv():
    shape = (2,3,4)
    nenv = 1
    venv = SubprocVecEnv([lambda: SimpleEnv(0, shape, u'float32')] * nenv)
    ob = venv.reset()
    venv.close()
